...
def resize_organized_pc(organized_pc, target_size):
    torch_organized_pc = torch.tensor(organized_pc).permute(2, 0, 1).unsqueeze(dim=0).contiguous()
    if target_size:
        torch_resized_organized_pc = torch.nn.functional.interpolate(torch_organized_pc, size=(target_size, target_size),                                                       mode='bicubic')
        return torch_resized_organized_pc.squeeze(dim=0)
    else:
        return torch_organized_pc
    # if tensor_out:
