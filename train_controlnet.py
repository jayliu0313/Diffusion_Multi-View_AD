...
class trainControlnet():
    ...
    def get_fc_image(self, latents):
        fc = self.decomp.get_fc(latents)
        latents = self.decomp.fuse_both(fc)
        fc_image = self.latents2image(latents)
        return fc_image
    ...
    def log_validation(self, text_embedding):
        ...
        for lightings, nmaps in tqdm(self.val_dataloader, desc="Validation"):
            ...
            with torch.no_grad():
                ...
                #encoder_hidden_states = lighting_text_embedding # [bs * 6, 77, 768]
                # fc_image = self.get_fc_image(latents)
                # Training ControlNet
